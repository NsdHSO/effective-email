...
export class ChatService {
  public chatSubject = new BehaviorSubject(
    {} as WrapperChat);
    ...
    this._httpClient.get<WrapperChat>(
      ...
      .subscribe(resp => {
        if(this.chatSubject.getValue().email?.messages !== undefined) {
          resp.email.messages = [
            ...this.chatSubject.getValue().email?.messages,
            ...resp.email?.messages
          ];
        }
  ...
  public sendMessage(
    ...
    return this._httpClient.put<WrapperChat>(
      `${this.environment.api}/chat/message/${chatId}`,
      bodyMessage
    )
      .pipe(
        tap(() => this.getChatById(
          chatId)))
      .subscribe();
  }
}
