...
export class OnScrollDirective implements OnInit, OnDestroy {
  ...
  scroll = (e : Event | any) => {
    ...
    //e.target.scrollHeight - e.target.offsetHeight - to calculate the bottom. equivalent to
    // scrollTopMax in FF
    console.log(e.target?.scrollTop,e.target.scrollHeight - e.target.offsetHeight);
    if(Math.trunc(e.target?.scrollTop+1) === e.target.scrollHeight - e.target.offsetHeight) {
  ...
  constructor(private ngZone : NgZone, private _elementRef : ElementRef) {
